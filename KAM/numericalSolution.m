...
function [tTotal,yTotal,impactInd] = numericalSolutionImp(IC,params,tSpan)
...
while true
    ...
    if tToEnd < tLim
        ...
        [tEnd,yEnd,te,~,ie] = ode45(@(t,x)rocking_block(t,x,IC,params),time, IC,options);  
        impactTime = te(ie==1);
    ...
    [t,y,te,~,ie] = ode45(@(t,x)rocking_block(t,x,IC,params),time, IC,options);
    impactTime = te(ie ==1);
    numInd = size(t,1);
