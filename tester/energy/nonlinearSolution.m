...
while ~stop
    ...
    if timeToEnd < tLim
        ...
        [tEnd,yEnd,crossTime,~,~] = ode45(@(t,x)nonlinearODE(t,x,IC,params),time,IC,options);
    ...
    options = odeset('Events',@eventFcn,'RelTol',1e-13,'AbsTol',1e-15);
    [t,y,crossTime,~,~] = ode45(@(t,x)nonlinearODE(t,x,IC,params),time,IC,options);
    ...
    [t,y,~,~,~] = ode45(@(t,x)nonlinearODE(t,x,IC,params),tVec,IC,options);
...
function  dx=nonlinearODE(~,x,IC,params)
