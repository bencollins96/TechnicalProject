...
while true
    ...
    if tToEnd < tLim
        time = linspace(0,tToEnd,1000);
        options = odeset('Events',@eventFcn,'RelTol',1e-8,'AbsTol',1e-10);
        ...
        if isempty(impactTime)
            yTotal(1+curInd:curInd + 1000,:) = yEnd;
            tTotal(1+curInd:curInd + 1000)   = tEnd + currentTime;
            curInd = curInd + 1000; 
            if any(ie==2)
