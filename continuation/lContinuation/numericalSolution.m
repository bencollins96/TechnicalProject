...
while true
    ...
    if tToEnd < tLim
        time = linspace(0,tToEnd,numPoints);
        ...
        if isempty(impactTime)
            yTotal(1+curInd:curInd + numPoints,:) = yEnd;
            tTotal(1+curInd:curInd + numPoints)   = tEnd + currentTime;
            curInd = curInd + numPoints; 
            if any(ie==2)
    ...
    time = linspace(0,tLim,numPoints);
    options = odeset('Events',@eventFcn,'RelTol',1e-10,'AbsTol',1e-12);
